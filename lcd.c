...
// Note timing changes for new version of sleep_us ()

  gpio_put (lcd->strbPin, 1) ; sleep_us (50) ;
  gpio_put (lcd->strbPin, 0) ; sleep_us (50) ;
}
...
static void sendDataCmd (const struct lcdDataStruct *lcd, unsigned char data)
{
  ...
  else
  {
    for (i = 0 ; i < 8 ; ++i)
    {
      gpio_put (lcd->dataPins [i], (myData & 1)) ;
      myData >>= 1 ;
    }
  }
  strobe (lcd) ;
...
static void put4Command (const struct lcdDataStruct *lcd, unsigned char command)
{
  ...
  for (i = 0 ; i < 4 ; ++i)
  {
    gpio_put (lcd->dataPins [i], (myCommand & 1)) ;
    myCommand >>= 1 ;
  }
  strobe (lcd) ;
}
...
// Create a new LCD:
  ...
  gpio_init(lcd->rsPin); 
   gpio_set_dir (lcd->rsPin,   GPIO_OUT) ;
   gpio_put (lcd->rsPin,   0) ;
  gpio_init(lcd->strbPin); 
   gpio_set_dir (lcd->strbPin, GPIO_OUT) ;
   gpio_put (lcd->strbPin, 0) ; 

  for (i = 0 ; i < bits ; ++i)
  {
    gpio_init(lcd->dataPins [i]);
    gpio_set_dir (lcd->dataPins [i], GPIO_OUT) ;
    gpio_put (lcd->dataPins [i], 0) ;
  }
  sleep_ms (35) ; // mS
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  if (bits == 4)
  {
    ...
    put4Command (lcd, func >> 4) ; sleep_ms (35) ;
    put4Command (lcd, func >> 4) ; sleep_ms (35) ;
    put4Command (lcd, func >> 4) ; sleep_ms (35) ;
  ...
  else
  {
    ...
    putCommand  (lcd, func     ) ; sleep_ms (35) ;
    putCommand  (lcd, func     ) ; sleep_ms (35) ;
    putCommand  (lcd, func     ) ; sleep_ms (35) ;
  }
